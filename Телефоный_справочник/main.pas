...
uses
  Classes, SysUtils, Forms, Controls, Graphics, Dialogs, Buttons, Grids, Edit;
...
procedure TTelefon.bAddClick(Sender: TObject);
begin
  fEdit.eName.Text:= '';
...
procedure TTelefon.bDelClick(Sender: TObject);
begin
...
procedure TTelefon.bEditClick(Sender: TObject);
begin
...
procedure TTelefon.bSortClick(Sender: TObject);
begin
  //если данных в сетке нет - просто выходим:
  if SG.RowCount = 1 then exit;
...
procedure TTelefon.FormClose(Sender: TObject; var CloseAction: TCloseAction);
var
...
procedure TTelefon.FormCreate(Sender: TObject);
var
  MyCont: Contacts; //для очередной записи
  f: file of Contacts; //файл данных
  i: integer; //счетчик цикла
